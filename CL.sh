...
echo "\n================ EXPAT COMPLATION =================\n"
./expat-2.5.0/configure --enable-static --host=$ENV_HOST && make install && cd .. 

echo "\n================ neon COMPLATION =================\n"
export LDFLAGS=-Wl,-L/usr/local/musl/lib,-lexpat &&
export CPPFLAGS=-I/usr/local/musl/include &&
./neon-0.31.0/configure --prefix=/usr/local/musl --with-expat --enable-static --host=$HOST && make install &&
